...
<body class="bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center min-h-screen p-4">
    <div class="bg-white p-8 md:p-12 rounded-xl shadow-2xl w-full max-w-md transform transition-all duration-300 hover:scale-[1.01] border border-gray-200">
        <div class="text-center mb-8">
            <h1 class="text-4xl font-extrabold text-gray-900 mb-2">Tulisin</h1>
            <p class="text-gray-600 text-lg">Web Kanal Artikel</p>
        </div>
        ...
        <form action="{{ route('actionlogin') }}" method="post">
            ...
            <div class="mb-6"> <label for="password" class="block mb-2 text-sm font-semibold text-gray-700">Password</label>
            ...
            {{-- Uncomment the following div to enable the "Remember me" checkbox --}}
            {{-- <div class="flex items-start mb-6">
                <div class="flex items-center h-5">
                    <input id="remember" type="checkbox" name="remember" class="w-4 h-4 border border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-blue-300 text-blue-600">
                </div>
                <label for="remember" class="ml-2 text-sm font-medium text-gray-900">Ingat saya</label>
            </div> --}}

            <button type="submit" class="w-full text-white bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-semibold rounded-lg text-base px-5 py-3.5 text-center transition duration-300 ease-in-out transform hover:-translate-y-0.5 shadow-lg hover:shadow-xl">Log In</button>
        </form>

        <hr class="my-6 border-gray-200">
        <p class="text-sm text-center text-gray-600">
            Belum punya akun? <a href="/register" class="font-medium text-blue-600 hover:underline hover:text-blue-800 transition duration-200">Register</a> sekarang!
        </p>
    </div>
        ...
        document.addEventListener('DOMContentLoaded', (event) => {
            ...
            if (alertElement) {
                setTimeout(() => {
                    ...
                    alertElement.style.transform = 'translateY(-10px)'; // Add a slight upward movement
